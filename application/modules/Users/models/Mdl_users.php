...
class Mdl_users extends CI_Model {
	...
	//Final Deals Comments Record
    public function _this_comment_record($form_id)
    {
	  ...
	  $query =  $this->db->get('comment_list');
        return $query->result(); 
    }		
	 
	//Final Deals Comments Record Single
    public function _this_single_comment_record($comment_id)
    {
	...
	public function _this_controller_sent_list_record ()
    {
		...
		{			
			...
			$where = "(`isCustomer` = '2') AND (`is_draft` = '0' OR `is_draft` = '2')";
        ...
        if($this->session->userdata('user_type') == 3)
		{
			...
			$where = "(`isManager` = '2') AND (`is_draft` = '0' OR `is_draft` = '2')";		
			$this->db->or_where($where);
        ...
        if($this->session->userdata('user_type') == 5)
		{
			...
			$where = "(`isAdmin` = '2') AND (`is_draft` = '0' OR `is_draft` = '2')";
			$this->db->where($where);
		}
