...
class Users extends MX_Controller  {
    ...
    public function deal_search()
    {
        ...
        $fee_status = $data_post['fee_status'];
        $id1 = $data_post['id1'];
        $campaign = $data_post['campaign'];
        ...
        if($fee_status != "")
        {
            $this->db->where('fee_status',$fee_status);
        }
         if($id1 != "")
        {
        ...
        $result_set =  $this->db->get('user_list')->result();
  ...
  <div class="row">
                            ...
                            <thead>
                                <tr>
									...
									<?php
										...
										if($this->session->userdata('view_deal_comment') != ""){
									?>
									 <th>Comments</th>
									 <?php
										}
                            ...
                            <tbody>
                                <?php foreach($result_set as $key => $row) { ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $row->deal_status;?></td>
                                        <td><?php echo $row->deal_status_date;?></td>
                                        <td><?php echo $row->fee_status;?></td>
                                        <td><?php echo $row->fee_status_date;?></td>
				 ...
				 <option value="">--Select--</option>
                    ...
                    <option value="Error" <?= $row->fee_status == 'Error' ? 'selected' : '' ?> >Error</option>
										...
										?>
										 ...
										 <td>
                                        <a class="btn btn-xs green" target="_blank" href="<?php echo base_url('Users/comment_list/'.$row->id.''); ?>">
                                                    Comments
                                                </a>
                                        </td>
										<?php
	 ...
	 var oTable1_deal = $('#deal_form').DataTable( {               
                    "aoColumns": [
                            { "bSortable": true },
	...
	$data['result_set'] = $this->Mdl_users->_this_comment_record($form_id);
        $data['content']    = 'Users/comment_list';
        $this->load->view('Template/template',$data);
	}
	
	//Add Comments From
	public function comment_list_add()
	{
		$form_id = $this->uri->segment(3); 
	$data['result_set'] = $this->Mdl_users->_this_comment_record($form_id);
        $data['content']    = 'Users/comment_list_add';
        $this->load->view('Template/template',$data);
	}
	
	
	//Update Comments From
	public function comment_list_edit()
	{
		$comment_id = $this->uri->segment(3); 
		$form_id = $this->uri->segment(4); 
	$data['result_set'] = $this->Mdl_users->_this_single_comment_record($comment_id);
        $data['content']    = 'Users/comment_list_edit';
        $this->load->view('Template/template',$data);
	}
	...
	public function comment_action()
	{
		...
		if($post['comment_id'] == "")
		{
			$data['comment_detail']   = $post['comment_detail'];
			...
			$db_command = $this->Mdl_users->db_command($data,null,'comment_list');  
		...
		else
		{
			...
			//$db_command = $this->Mdl_users->db_command($data,$post['comment_id'],'comment_list');  
			//$db_command = $this->db->update('comment_list', $data,$post['comment_id']);   
			
			$this->db->where('comment_id',$post['comment_id']);
			$db_command = $this->db->update('comment_list',$data);
		 ...
		 if($post['comment_id'] != null)
            ...
            redirect(base_url().'Users/comment_list/'.$post['form_id'],'refresh');  
    ...
    public function update_status($type){
		...
		else
		{
			...
			$new_type = $type.'_date';
			   ...
			   $this->db->update('user_list', array($type => $value,$new_type => date('d/m/Y') ), array('id' => $id));
               echo '<div class="alert alert-success alert-dismissable">
                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                <strong>Success!</strong> Status Updated.
             </div>';
	...
	public function saved_action($id = null)
    { 
        ...
        $data['content']   = 'Users/saved_form_edit';
        $this->load->view('Template/template',$data);
...
// $html2pdf->WriteHTML($content); // in $content you put your content to have in pdf
  //  $html2pdf->Output('exemple.pdf');
	...
	public function update_saved_deals()
    { 
	    $post = $this->input->post();
...
//print_r($new_record);
            ...
            redirect(base_url().'Users/saved_list','refresh');  
        }  
