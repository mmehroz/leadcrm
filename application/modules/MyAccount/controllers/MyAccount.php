...
class MyAccount extends MX_Controller  {
    ...
    public function add(){
        ...
        $data['content']   = 'MyAccount/add_user';
        $this->load->view('Template/template',$data);
    }
	
	public function user_list(){
		 $data['result_set'] = $this->Mdl_myaccount->_this_controller_record();
        $data['content']   = 'MyAccount/user_list';
        $this->load->view('Template/template',$data);
    }
...
if($post['password'] != "")
{
	$pass = Modules::run('Login/_hash', $post['password']);
}else
{
	$pass = $post['old_password'];
}
                $data = array(
                        'account_title'     => $this->already_exists_row('user_type','id',$type,'type'),
                        'username'          => $username,                    
						'password'          => $pass,
						'mobile_no'         => $mobile_no,
                        'email'             => $email,
                        ...
                        'update_saved_deal'            => $update_saved_deal,
                        'view_saved_pdf'            => $view_saved_pdf,
                        'create_deal'            => $create_deal,
                        'saved_deals'            => $saved_deals,
                        'deals_home'            => $deals_home,
                        'view_deals_home'            => $view_deals_home,
                        'edit_deal_comment'            => $edit_deal_comment,
                        'view_deal_comment'            => $view_deal_comment,
                    ...
                    $return_id = $this->db->update('login', $data , array('id' => $post['id']));
                    

                    if(!empty($return_id)) {
                        $this->session->set_flashdata('save', 'user successfully Updated');
                    }
                    redirect(base_url() . 'MyAccount/user_list', 'refresh');

            //} else {
            //    $this->session->set_flashdata('error', 'Email field should be unique');
            //    redirect(base_url() . 'MyAccount/add/'.$post['id'], 'refresh');
           // }
			...
			else
			{
				...
				if($this->form_validation->run() == TRUE){
					...
					$data = array(
							'account_title'     => $this->already_exists_row('user_type','id',$type,'type'),
							'username'          => $username,
							'password'          => Modules::run('Login/_hash', $password),
							'mobile_no'         => $mobile_no,
							'email'             => $email,
							'user_type'         => $type,
							'center'            => $center,
						...
						$return_id = $this->db->insert('login', $data);
						

						if(!empty($return_id)) {
							$this->session->set_flashdata('save', 'New user successfully Added');
						}
						redirect(base_url() . 'MyAccount', 'refresh');

				} else {
					$this->session->set_flashdata('error', 'Email field should be unique');
					redirect(base_url() . 'MyAccount/add', 'refresh');
				}
			}
        }
    }
